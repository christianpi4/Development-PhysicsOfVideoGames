...
void ModulePhysics::ShapeBox(float x, float y, float w, float h, bool d = false)
{
	...
	if (d == true) {
		...
		b->CreateFixture(&rectangle, 1.0f);
	}

}

void ModulePhysics::ShapeChain(int s, float x, float y, bool d = false)
{
	...
	if(s==1){
		...
		chain1.position.Set(PIXEL_TO_METERS(x), PIXEL_TO_METERS(y));
		...
		if (d == true) {
			...
			// TODO 2: To have the box behave normally, set fixture's density to 1.0f
			b2FixtureDef fixture1;
			fixture1.shape = &chain;
			b2->CreateFixture(&fixture1);

		}
		else {
			b2->CreateFixture(&chain, 1.0f);
		}
