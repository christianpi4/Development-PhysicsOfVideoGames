...
update_status ModulePhysics::PostUpdate()
{
	...
	if(App->input->GetKey(SDL_SCANCODE_2) == KEY_DOWN)
	{
		...
		float height = PIXEL_TO_METERS(25);
	...
	if(App->input->GetKey(SDL_SCANCODE_3) == KEY_DOWN)
	{
		...
		for (int i = 0; i < 12; i++) {
			...
			buffer[i].Set(PIXEL_TO_METERS(points[counter]), PIXEL_TO_METERS(points[counter + 1]));
			counter += 2;
			LOG("POINTS %i", points[i]);
		}
		...
		b2ChainShape Chain;
		Chain.CreateLoop(buffer, 12);
		b2FixtureDef fixture3;
		fixture3.shape = &Chain;
